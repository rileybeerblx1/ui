	...
	closeHolder.InputBegan:Connect(function(input)
		if input.UserInputType == ui or input.UserInputType == Enum.UserInputType.Touch then
			...
			if subHolder then
				...
				tweenService:Create(round, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
					ImageColor3 = parentTable.open and Color3.fromRGB(12, 12, 16) or Color3.fromRGB(8, 8, 12)
				}):Play()
			end
...
local function createButton(option, parent)
	...
	main.InputBegan:Connect(function(input)
		if input.UserInputType == ui or input.UserInputType == Enum.UserInputType.Touch then
			createRipple(round, input.Position)
			library.flags[option.flag] = true
			clicking = true
			tweenService:Create(round, TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
				ImageColor3 = Color3.fromRGB(255, 70, 85)
			}):Play()
			option.callback()
		end
		if input.UserInputType == Enum.UserInputType.MouseMovement then
			inContact = true
			tweenService:Create(round, TweenInfo.new(0.12, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
	...
	main.InputEnded:Connect(function(input)
		if input.UserInputType == ui or input.UserInputType == Enum.UserInputType.Touch then
			clicking = false
			tweenService:Create(round, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
...
local function createBind(option, parent)
	...
	function option:SetKey(key)
		...
		round.Size = UDim2.new(0, -textService:GetTextSize(bindinput.Text, math.floor(13 * library.scale), Enum.Font.GothamMedium, Vector2.new(9e9, 9e9)).X - math.floor(18 * library.scale), 1, -math.floor(10 * library.scale))
	end
end
